...
namespace UnityEngine.UI
{
    ...
    /// <summary>
    /// 用于使子级 RectTransform 能够滚动的组件。
    /// </summary>
    /// <remarks>
    /// ScrollRect 本身不执行任何裁剪（clipping）。结合 Mask 或 RectMask2D 组件，它可以变成一个完整的滚动视图 (ScrollView)。
    /// </remarks>
    // [重点注释 - 复杂 UI 组件参考实现]
    // 这是 UGUI 中最复杂的组件之一，它同时实现了拖拽接口 (IDragHandler 等) 和布局接口 (ILayoutElement 等)。
    // 仔细阅读它的 LateUpdate 和拖拽实现，能学到惯性 (Inertia)、回弹 (Elastic) 的运动学计算，
    // 以及如何在复杂状态下处理 Layout 和输入事件的优雅写法。
    public class ScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        // [重点注释 - 惯性与回弹计算]
        // LateUpdate 用于处理当用户松开手之后的惯性滑动和边界回弹。
        // 选择在 LateUpdate 中执行是为了确保在本帧所有其它输入和 Layout 均已完成之后，再计算出最终的位置。
