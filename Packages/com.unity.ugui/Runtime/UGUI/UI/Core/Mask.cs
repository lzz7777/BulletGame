...
namespace UnityEngine.UI
{
    ...
    /// </summary>
    /// <remarks>
    /// 通过使用此元素，任何启用了遮罩的子元素都会根据该同级 Graphic 在模板缓冲区 (stencil buffer) 中写入非 0 值的位置进行显示。
    /// </remarks>
    // [重点注释 - 模板测试遮罩]
    // 它是通过修改材质的模板缓冲区 (Stencil Buffer) 来实现任意形状裁剪的。
    // 代价：
    // 1. 会导致增加 DrawCall（因为需要额外的绘制 Pass 来写入/清除模板缓存）。
    // 2. 它依赖于 Graphic 组件（通常是 Image）提供的形状，可以实现圆形、异形裁剪。
