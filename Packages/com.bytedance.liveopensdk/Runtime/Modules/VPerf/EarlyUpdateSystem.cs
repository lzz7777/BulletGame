...
namespace ByteDance.LiveOpenSdk.Perf
{
    ...
    /// </summary>
    internal struct EarlyUpdateSystem
    {
        ...
        private static void InsertPlayerLoopSystemBefore(Type systemType, Type anchorSystem, PlayerLoopSystem subSystem)
        {
            ...
            if (systemIndex == -1)
            {
                Debug.LogError("CloudGameSystemEarlyUpdateSystem - System not found: " + systemType);
                return;
            }
            ...
            if (anchorIndex == -1)
            {
                Debug.LogError("CloudGameSystemEarlyUpdateSystem - Anchor System not found: " + anchorSystem);
